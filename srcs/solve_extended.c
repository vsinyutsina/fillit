...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/17 16:58:08 by akelli            #+#    #+#             */
/*   Updated: 2020/01/17 16:58:11 by akelli           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		shift_extended(t_tetramino *figure, t_border map_border)
{
	...
	{
		figure->map_extended = (figure->map_extended >> 1)
				| (figure->map << 127);
		figure->map >>= 1;
	}
	else
	{
		figure->map_extended = (figure->map_extended >> figure->width)
				| (figure->map << (128 - figure->width));
...
{
	if (figure->map == figure->map_begin && other_figures.last[figure->type])
	{
		figure->map = other_figures.last[figure->type];
		figure->map_extended = other_figures.last_extended[figure->type];
	}
